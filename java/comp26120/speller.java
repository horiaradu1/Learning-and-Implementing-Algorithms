...
public abstract class speller {
    ...
    public char[] get_next_lower_word(BufferedReader source) 
    {
	...
	try {
		...
		while (!done) {
			...
			words.insert(new String(speller.word));
			speller.word_count++;
    ...
    public int process_args(String[] args) {
        ...
        Getopt g = new Getopt("speller", args, "s:d:m:vh");
	String optarg;
        ...
        int c= g.getopt();
         if (c == -1)
	      ...
	      break;
          case 'v':
	      config.verbose++; // multiple -v args will increase this
	      break;
          case 'h': // Help!
	      usage ();
	      break;
          default: // report error
              System.out.format("didn't expect program parameter %c [0%o]\n", c, c);
            usage ();
          }
        }
